...
```python
registry.register_toolset(EditorToolSet(workspace_root="/tmp/work"))
```

Tools from a toolset are automatically namespaced: `toolset_name.tool_name`. You can override the namespace:

```python
registry.register_toolset(EditorToolSet(...), namespace="editor")
# registers as editor.read_file, editor.write_file, etc.
```
