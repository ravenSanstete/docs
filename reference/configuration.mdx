---
title: "Configuration"
description: "All configuration options available in QitOS: config.yaml, AgentModule.run() arguments, Engine constructor arguments, and environment variables."
---

QitOS is configured through four overlapping layers, each narrowing in scope:

1. **`config.yaml`** — file-level defaults loaded by agent templates
2. **`AgentModule.run()` keyword arguments** — per-run overrides
3. **`Engine` constructor keyword arguments** — fine-grained execution knobs
4. **Environment variables** — secrets and deployment-level settings

---

## config.yaml

Agent templates read a `config.yaml` from the project directory. Below is the full set of recognized keys with their defaults.

```yaml
name: qitos_default        # Agent/run name; used as the trace prefix
max_steps: 12              # Maximum steps per run (overridable at runtime)

model:
  provider: openai_compatible   # Model backend: "openai_compatible" | "openai" | "azure" | "ollama" | ...
  base_url: https://api.siliconflow.cn/v1/   # API base URL
  api_key: ${OPENAI_API_KEY}                 # API key; supports ${ENV_VAR} interpolation
  model: Qwen/Qwen3-8B                       # Model identifier sent to the API
  model_name: Qwen/Qwen3-8B                  # Human-readable model name (used in traces)
...
- `name` becomes the prefix for auto-generated run IDs when `trace_prefix` is not set.
- `max_steps` is the default budget. You can override it per-run with `AgentModule.run(max_steps=...)`.
- `api_key` supports `${VAR}` syntax for environment variable substitution at load time.

---

## AgentModule.run() arguments

All of the following can be passed to `agent.run(task, ...)`. They override any defaults from `config.yaml` or the `Engine` constructor.
...
|----------|----------|-------------|
| `OPENAI_API_KEY` | Yes (for OpenAI / compatible APIs) | API key sent in the `Authorization` header. Used by `OpenAIModel`, `OpenAICompatibleModel`, and as the default `api_key` in `config.yaml` |
