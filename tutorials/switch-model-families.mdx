...
1. family preset (a reusable configuration bundle for a model family)
2. harness policy (the wiring layer that connects a transport, parser, and protocol)
3. model transport (the adapter that sends requests to and receives responses from a model API)
4. parser (a component that converts raw model output into a typed Decision) and protocol (the output format the model is asked to follow)
