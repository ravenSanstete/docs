...
The full prompt in the example also lists preferred tool patterns and ends with the ReAct output contract:
...
## The parser remains ReAct on purpose
...
without changing the parser or protocol.

## The model harness is still text-first

The example still uses `OpenAICompatibleModel`. That means:

- the model returns text
- the tool schema is injected into the prompt
- the parser turns text into a `Decision`

This remains a strong default for research because it is:

- easy to compare across providers
- easy to inspect in traces
- easy to adapt to local endpoints

If you later need native tool-call protocols, QitOS can support them, but do not pay that coupling cost until it buys you something.
