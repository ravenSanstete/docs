...
This lesson is the first time you bend the default loop.
...
Lesson 2 already teaches a subtle but important QitOS idea:
...
<Steps>
  <Step title="Extend state with a plan and a cursor">
    ...
    This is the first time the course makes a hidden reasoning artifact (a persistent output produced by a run) explicit.
  ...
  <Step title="Use a dedicated plan builder">
    ...
    planning becomes a named artifact with a dedicated parser (a component that converts raw model output into a typed Decision), not an unstructured paragraph in the main scratchpad.
  </Step>
  ...
  <Step title="Use decide only as the planning gate">
    ...
    Lesson 2 is not about replacing the runtime. It is about adding one explicit control boundary to it.
  </Step>
